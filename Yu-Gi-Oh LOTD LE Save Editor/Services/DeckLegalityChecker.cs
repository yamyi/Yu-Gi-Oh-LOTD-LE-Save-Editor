...
/// Limited copy, or more than 2 Semi-Limited copies of the same card, per
/// LotdBanlist (the curated, lotd_id-keyed list - not Cards.json's own
/// generic banlist_info field, which didn't match this format). Counts are
/// tallied by Card.Id (passcode) across Main+Extra+Side combined, matching
/// the same 3-copies-per-card rule Deck Editor enforces when adding cards.
/// </summary>
public static class DeckLegalityChecker
{
    ...
    private static string? FindBanlistViolation(SlotIO.SlotInfo slot)
    {
        ...
        foreach (var (cardId, count) in counts)
        {
            ...
            string? status = LotdBanlist.GetStatus(card?.LotdId);
